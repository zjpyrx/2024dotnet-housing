  ...
  <el-container>
    ...
    <el-container>
      <el-aside width="350px">
        ...
        ></el-button>
        <el-button @click="getNearestCommunities">获取附近中介</el-button>
  ...
  methods: {
    ...
    async searchCommunities() {
      try {
        const response = await axios.get(
          "http://localhost:18101/api/v1/agents/search",
          { params: { keyword: this.keyword } }
        );
        this.agentData = response.data.map((agent) => ({
    ...
    async getNearestCommunities() {
      try {
        ...
        const response = await axios.get(
          "http://localhost:18101/api/v1/agents/nearest",
    ...
    displayOnMap() {
      ...
      const curpoint = new BMap.Point(this.agentData[0].point.lng, this.agentData[0].point.lat);
      this.map.centerAndZoom(curpoint, 15);
      this.openInfoWindow(this.agentData[0]);
    },
    openInfoWindow(poi) {
